...
def _errno_ptr(out result: Pointer[c_int, MutUntrackedOrigin]):
    comptime if CompilationTarget.is_linux():
        ...
        # The UCRT's per-thread errno accessor, the counterpart of glibc's
        # __errno_location. `errno` itself is a macro expanding to `*_errno()`,
        # so there is no symbol of that name to call.
